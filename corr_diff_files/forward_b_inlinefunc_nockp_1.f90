...
CONTAINS
!  Differentiation of forward_problem in reverse (adjoint) mode, forward sweep (with options split(forward_problem) OpenMP):
...
!  Differentiation of forward_problem in reverse (adjoint) mode, backward sweep (with options split(forward_problem) OpenMP):
!   gradient     of useful results: v
!   with respect to varying inputs: v xx
...
!    xxb = 0.0_8
    ...
    DO i=chunk_end,chunk_start,-1
      ...
      IF (branch .NE. 0) THEN
        ...
        tempb = xx(i)*vib(i)
        CALL POPREAL8(temp)
        CALL DUMMY_FUNC_B(xx(i), xxb(i), tempb)
        xxb(i) = xxb(i) + result1*vib(i)
        result1b = xx(i)*vib(i)
        vib(i) = 0.0_8
        CALL POPREAL8(result1)
        CALL DUMMY_FUNC_B(xx(i), xxb(i), result1b)
      END IF
    END DO
!$OMP END PARALLEL
    vb = 0.0_8
...
!  Differentiation of dummy_func in reverse (adjoint) mode (with options split(forward_problem) OpenMP):
!   gradient     of useful results: x dummy_func
!   with respect to varying inputs: x
  SUBROUTINE DUMMY_FUNC_B(x, xb, dummy_funcb)
    IMPLICIT NONE
    REAL*8 :: dummy_func
    REAL*8 :: dummy_funcb
    REAL*8, INTENT(IN) :: x
