!        Generated by TAPENADE     (INRIA, Ecuador team)
!  Tapenade 3.16 (develop) - 14 Mar 2025 11:54
!
MODULE FORWARD_DIFF
  IMPLICIT NONE

CONTAINS
!  Differentiation of forward_problem in reverse (adjoint) mode (with options OpenMP):
!   gradient     of useful results: v
!   with respect to varying inputs: v xx
!   RW status of diff variables: v:in-zero xx:out
  SUBROUTINE FORWARD_PROBLEM_B(xx, xxb, v, vb)
    IMPLICIT NONE
    INTEGER :: i
    REAL*8, DIMENSION(10), INTENT(IN) :: xx
    REAL*8, DIMENSION(10) :: xxb
    REAL*8, DIMENSION(10) :: vi
    REAL*8, DIMENSION(10) :: vib
    REAL*8 :: v
    REAL*8 :: vb
    INTRINSIC SIN
    INTRINSIC ABS
    INTRINSIC MAX
    INTRINSIC SUM
    REAL*8 :: x1
    REAL*8 :: x1b
    REAL*8 :: abs0
    REAL*8 :: abs0b
    REAL*8 :: max1
    REAL*8 :: max1b
    REAL*8 :: abs1
    REAL*8 :: abs1b
    REAL*8 :: max2
    REAL*8 :: max2b
    INTEGER*4 :: branch
    INTEGER :: chunk_start
    INTEGER :: chunk_end
!$OMP PARALLEL DEFAULT(shared), PRIVATE(i), PRIVATE(abs0, max1, abs1, max2, chunk_start, chunk_end), PRIVATE(x1)
    CALL GETSTATICSCHEDULE(1, 10, 1, chunk_start, chunk_end)
    DO i=chunk_start,chunk_end
      IF (xx(i)**2 + xx(i)**3 .GE. 0) THEN
! sin function
        vi(i) = SIN(xx(i))
        IF (xx(i) .GE. 0.) THEN
          abs0 = xx(i)
          CALL PUSHCONTROL1B(0)
        ELSE
          abs0 = -xx(i)
          CALL PUSHCONTROL1B(1)
        END IF
! abs function
        vi(i) = vi(i) + abs0
        IF (xx(i) .LT. 1.0) THEN
          CALL PUSHCONTROL1B(0)
          max1 = 1.0
        ELSE
          max1 = xx(i)
          CALL PUSHCONTROL1B(1)
        END IF
! max function
        vi(i) = vi(i) + max1
        IF (xx(i) .GE. 0.) THEN
          CALL PUSHREAL8(abs1)
          abs1 = xx(i)
          CALL PUSHCONTROL1B(0)
        ELSE
          CALL PUSHREAL8(abs1)
          abs1 = -xx(i)
          CALL PUSHCONTROL1B(1)
        END IF
        IF (xx(i) .GE. 0.) THEN
          x1 = xx(i)
          CALL PUSHCONTROL1B(0)
        ELSE
          x1 = -xx(i)
          CALL PUSHCONTROL1B(1)
        END IF
        IF (x1 .LT. 1.0) THEN
          CALL PUSHREAL8(max2)
          max2 = 1.0
          CALL PUSHCONTROL1B(0)
        ELSE
          CALL PUSHREAL8(max2)
          max2 = x1
          CALL PUSHCONTROL1B(1)
        END IF
! abs and max together
        vi(i) = vi(i) + abs1/max2
        CALL PUSHCONTROL1B(1)
      ELSE
        CALL PUSHCONTROL1B(0)
      END IF
    END DO
    CALL PUSHREAL8(abs1)
    CALL PUSHREAL8(max2)
!$OMP END PARALLEL
    vib = 0.0_8
    vib = vb
    xxb = 0.0_8
!$OMP PARALLEL DEFAULT(shared), PRIVATE(i), PRIVATE(abs0, max1, abs1, max2, branch, chunk_end, chunk_start), PRIVATE(abs0b, max1b, abs1b, max2b, x1, x1b)
    CALL POPREAL8(max2)
    CALL POPREAL8(abs1)
!    xxb = 0.0_8
    CALL GETSTATICSCHEDULE(1, 10, 1, chunk_start, chunk_end)
    DO i=chunk_end,chunk_start,-1
      CALL POPCONTROL1B(branch)
      IF (branch .NE. 0) THEN
        abs1b = vib(i)/max2
        max2b = -(abs1*vib(i)/max2**2)
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) THEN
          CALL POPREAL8(max2)
          x1b = 0.0_8
        ELSE
          CALL POPREAL8(max2)
          x1b = max2b
        END IF
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) THEN
          xxb(i) = xxb(i) + x1b
        ELSE
          xxb(i) = xxb(i) - x1b
        END IF
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) THEN
          CALL POPREAL8(abs1)
          xxb(i) = xxb(i) + abs1b
        ELSE
          CALL POPREAL8(abs1)
          xxb(i) = xxb(i) - abs1b
        END IF
        max1b = vib(i)
        CALL POPCONTROL1B(branch)
        IF (branch .NE. 0) xxb(i) = xxb(i) + max1b
        abs0b = vib(i)
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) THEN
          xxb(i) = xxb(i) + abs0b
        ELSE
          xxb(i) = xxb(i) - abs0b
        END IF
        xxb(i) = xxb(i) + COS(xx(i))*vib(i)
        vib(i) = 0.0_8
      END IF
    END DO
!$OMP END PARALLEL
    vb = 0.0_8
  END SUBROUTINE FORWARD_PROBLEM_B

  SUBROUTINE FORWARD_PROBLEM(xx, v)
    IMPLICIT NONE
    INTEGER :: i
    REAL*8, DIMENSION(10), INTENT(IN) :: xx
    REAL*8, DIMENSION(10) :: vi
    REAL*8, INTENT(OUT) :: v
    INTRINSIC SIN
    INTRINSIC ABS
    INTRINSIC MAX
    INTRINSIC SUM
    REAL*8 :: x1
    REAL*8 :: abs0
    REAL*8 :: max1
    REAL*8 :: abs1
    REAL*8 :: max2
!$OMP PARALLEL DO DEFAULT(shared), PRIVATE(i), PRIVATE(abs0, max1, abs1, max2), PRIVATE(x1), SCHEDULE(static)
    DO i=1,10
      IF (xx(i)**2 + xx(i)**3 .GE. 0) THEN
! sin function
        vi(i) = SIN(xx(i))
        IF (xx(i) .GE. 0.) THEN
          abs0 = xx(i)
        ELSE
          abs0 = -xx(i)
        END IF
! abs function
        vi(i) = vi(i) + abs0
        IF (xx(i) .LT. 1.0) THEN
          max1 = 1.0
        ELSE
          max1 = xx(i)
        END IF
! max function
        vi(i) = vi(i) + max1
        IF (xx(i) .GE. 0.) THEN
          abs1 = xx(i)
        ELSE
          abs1 = -xx(i)
        END IF
        IF (xx(i) .GE. 0.) THEN
          x1 = xx(i)
        ELSE
          x1 = -xx(i)
        END IF
        IF (x1 .LT. 1.0) THEN
          max2 = 1.0
        ELSE
          max2 = x1
        END IF
! abs and max together
        vi(i) = vi(i) + abs1/max2
      END IF
    END DO
    v = SUM(vi)
  END SUBROUTINE FORWARD_PROBLEM

END MODULE FORWARD_DIFF

