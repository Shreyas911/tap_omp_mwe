...
!$OMP END PARALLEL
    ...
    vib = vb
    xxb = 0.0_8
!$OMP PARALLEL DEFAULT(shared), PRIVATE(i, temp), PRIVATE(tempb), PRIVATE(result1, branch, chunk_end, chunk_start), PRIVATE(result1b)
    CALL POPREAL8(temp)
...
!$OMP END PARALLEL
  ...
  SUBROUTINE FORWARD_PROBLEM(xx, v)
    ...
    INTRINSIC SUM
    REAL*8 :: result1
!$OMP PARALLEL DO DEFAULT(shared), PRIVATE(i, temp), PRIVATE(result1), SCHEDULE(static)
    DO i=1,10000
      IF (xx(i)**2 + xx(i)**3 .GE. 0) THEN
! Inline function call
        result1 = DUMMY_FUNC(xx(i))
...
!  Differentiation of dummy_func in reverse (adjoint) mode (with options OpenMP):
