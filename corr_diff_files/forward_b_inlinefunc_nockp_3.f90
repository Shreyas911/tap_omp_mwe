...
CONTAINS
!  Differentiation of forward_problem in reverse (adjoint) mode, forward sweep (with options split(forward_problem dummy_func) Op
!enMP):
!   gradient     of useful results: v
!   with respect to varying inputs: v xx
  SUBROUTINE FORWARD_PROBLEM_FWD(xx, v)
    IMPLICIT NONE
    INTEGER :: i
...
!$OMP END PARALLEL
    CALL PUSHREAL8(result1)
    CALL PUSHREAL8(temp)
  END SUBROUTINE FORWARD_PROBLEM_FWD

!  Differentiation of forward_problem in reverse (adjoint) mode, backward sweep (with options split(forward_problem dummy_func) O
!penMP):
!   gradient     of useful results: v
!   with respect to varying inputs: v xx
!   RW status of diff variables: v:in-zero xx:out
  SUBROUTINE FORWARD_PROBLEM_BWD(xx, xxb, v, vb)
    IMPLICIT NONE
    INTEGER :: i
    REAL*8, DIMENSION(10), INTENT(IN) :: xx
    ...
    vib = vb
    xxb = 0.0_8
!$OMP PARALLEL DEFAULT(shared), PRIVATE(i, temp), PRIVATE(tempb, result1b), PRIVATE(result1, branch, chunk_end, chunk_start)
    CALL POPREAL8(temp)
    CALL POPREAL8(result1)
    tempb = 0.0_8
!    xxb = 0.0_8
...
!$OMP END PARALLEL
    vb = 0.0_8
  END SUBROUTINE FORWARD_PROBLEM_BWD
...
!  Differentiation of dummy_func in reverse (adjoint) mode, forward sweep (with options split(forward_problem dummy_func) OpenMP)
!:
...
!  Differentiation of dummy_func in reverse (adjoint) mode, backward sweep (with options split(forward_problem dummy_func) OpenMP
!):
!   gradient     of useful results: x dummy_func
!   with respect to varying inputs: x
